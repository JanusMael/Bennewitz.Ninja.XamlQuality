...
| **The guidance documents never name the repository or session that sent an entry.** This covers `avalonia-gotchas.md`, `ai-drivable-ui.md` and `ai-drivable-ui/` | They say what was measured and how: the versions, the platform, the method. They never say who measured it. The owner's rule, 2026-09-24. Where a contribution came from belongs in the commit, the pull request and `PROGRESS.md`. A fact about the family's own packages, such as a version they require, is not a credit and stays |
