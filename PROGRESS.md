...
Everything after the `v2026.3.922` tag (`5d49915`). Most of it makes a rule stricter or adds API,
so the release that carries it has to say so.
...
| `e92fb92` | `docs/ai-drivable-ui.md`, TailBlazer's guide to a UI an agent can drive and verify, copied as written | Docs only |
...
| A custom control that carries an automation id or name but has no peer | `ai-drivable-ui.md`, rule 3 | Possible | The element's type, in the scanned assemblies, has no `OnCreateAutomationPeer` of its own | Types, as XQ1003 does, plus the framework's peer-less base types named |
...
1. **Which of the candidates to write, and in what order.** `XQ1005` is already decided. The suggested
   order after it is the explicit `AutomationId`, then the zero-size slot, then the peer check, then
   the container half of the naming rule.
