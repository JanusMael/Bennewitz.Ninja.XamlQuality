...
| PR #14 | `docs/ai-drivable-ui/` arrives with TailBlazer's harness artefacts, each adapted and approved by the owner: `Probe-UiTestEnvironment.ps1`, the shared-desktop and Windows Sandbox journals, and `HarnessWindowPlacement.cs` as a worked example. The guide links to each, gains two points on sweeps, and no longer says an agent-started window never holds the foreground | Docs only |
