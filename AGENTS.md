...
- **`XQ1006`** reports a themed custom control that keeps the framework's empty automation peer,
  which a control-view search never reaches, so the names `XQ1001` and `XQ1002` require reach
...
| `XQ1003`, `XQ1005` and `XQ1006` never throw on a consumer's build output. A type whose dependencies do not load is unknown, never unfocusable, and one a scanned assembly defines is named in `Skipped` with what stopped it, never taken for a framework type or for one the scan was not given | One missing dependency fails the whole scan, a consumer's own control passes for a framework control, or a skip tells the consumer to pass the assembly they passed | `LoadedTypes`; `ElementTypes.IsUnreadable`, `ElementTypes.WhyUnresolved`; `TemplatePartRule` (its private `PartNamesIn` and `OwnerOf`); `KeyBindingFocusRule.Analyze`; `CustomControlPeerRule.Analyze`; tests `UnloadableBuildOutputTests.XQ1003_AControlThatDoesNotLoad_IsNamedWithWhatStoppedIt`, `UnloadableBuildOutputTests.XQ1003_AMethodWhoseLocalsDoNotLoad_CostsOnlyItself`, `UnloadableBuildOutputTests.XQ1003_ANestedTypeWhoseOwnerDoesNotLoad_DoesNotThrow`, `UnloadableBuildOutputTests.XQ1005_AControlThatDoesNotLoad_SaysSo_NotThatItWasNotPassed`, `UnloadableBuildOutputTests.XQ1006_AControlThatDoesNotLoad_IsNamedWithWhatStoppedIt`, `UnloadableBuildOutputTests.XQ1006_APeerMethodThatCannotBeRead_IsNamedNotThrown` |
| `XQ1006` reports a themed control only when the `OnCreateAutomationPeer` its type resolves to is the root declaration. Any override in its chain is a peer: the control's own, a base's, or an empty one returned on purpose, which is how a decorative control opts out | A control with a peer is reported, or a decorative control can be silenced only from outside its code | `CustomControlPeerRule` remarks and its private `PeerOf`; tests `CustomControlPeerRuleTests.APeerlessThemedControl_IsReported`, `CustomControlPeerRuleTests.AControlDerivedFromOneWithAPeer_IsClean`, `CustomControlPeerRuleTests.AControlOnAFrameworkBaseWithAPeer_IsClean`, `CustomControlPeerRuleTests.ADecorativeControlThatSaysSoInItsCode_IsClean` |
| `XQ1006` checks only a control the scanned assemblies define, once however often it is themed, and names in `Skipped` every themed control when given no assemblies, a name two scanned types carry, and a type with no `OnCreateAutomationPeer` | A framework control's theme is blamed, one defect is reported twice, or a control nothing read passes as clean | `CustomControlPeerRule.Analyze`; tests `CustomControlPeerRuleTests.AThemeForATypeTheScanWasNotGiven_IsNeitherCheckedNorNamed`, `CustomControlPeerRuleTests.AControlThemedTwice_IsReportedOnce`, `CustomControlPeerRuleTests.WithoutAssemblies_EveryThemedControlIsNamedAsSkipped`, `CustomControlPeerRuleTests.ANameTwoScannedTypesCarry_IsSkipped`, `CustomControlPeerRuleTests.ATypeWithNoPeerMethod_IsSkippedNotReported` |
