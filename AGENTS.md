...
| `BNXQ1003`, `BNXQ1005` and `BNXQ1006` never throw on a consumer's build output. A type whose dependencies do not load is unknown, never unfocusable, and one a scanned assembly defines is named in `Skipped` with what stopped it, never taken for a framework type or for one the scan was not given | One missing dependency fails the whole scan, a consumer's own control passes for a framework control, or a skip tells the consumer to pass the assembly they passed | `LoadedTypes`; `ElementTypes.IsUnreadable`, `ElementTypes.WhyUnresolved`; `TemplatePartRule` (its private `PartNamesIn` and `OwnerOf`); `KeyBindingFocusRule.Analyze`; `CustomControlPeerRule.Analyze`; tests `UnloadableBuildOutputTests.BNXQ1003_AControlThatDoesNotLoad_IsNamedWithWhatStoppedIt`, `UnloadableBuildOutputTests.BNXQ1003_AMethodWhoseLocalsDoNotLoad_CostsOnlyItself`, `UnloadableBuildOutputTests.BNXQ1003_ANestedTypeWhoseOwnerDoesNotLoad_DoesNotThrow`, `UnloadableBuildOutputTests.BNXQ1005_AControlThatDoesNotLoad_SaysSo_NotThatItWasNotPassed`, `UnloadableBuildOutputTests.BNXQ1006_AControlThatDoesNotLoad_IsNamedWithWhatStoppedIt`, `UnloadableBuildOutputTests.BNXQ1006_APeerMethodThatCannotBeRead_IsNamedNotThrown` |
