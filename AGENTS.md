...
| `XQ1003` and `XQ1005` never throw on a consumer's build output. A type whose dependencies do not load is unknown, never unfocusable, and one a scanned assembly defines is named in `Skipped` with what stopped it, never taken for a framework type or for one the scan was not given | One missing dependency fails the whole scan, a consumer's own control passes for a framework control, or a skip tells the consumer to pass the assembly they passed | `LoadedTypes`; `ElementTypes.IsUnreadable`, `ElementTypes.WhyUnresolved`; `TemplatePartRule` (its private `PartNamesIn` and `OwnerOf`); `KeyBindingFocusRule.Analyze`; tests `UnloadableBuildOutputTests.XQ1003_AControlThatDoesNotLoad_IsNamedWithWhatStoppedIt`, `UnloadableBuildOutputTests.XQ1003_AMethodWhoseLocalsDoNotLoad_CostsOnlyItself`, `UnloadableBuildOutputTests.XQ1003_ANestedTypeWhoseOwnerDoesNotLoad_DoesNotThrow`, `UnloadableBuildOutputTests.XQ1005_AControlThatDoesNotLoad_SaysSo_NotThatItWasNotPassed` |
