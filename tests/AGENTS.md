...
|---|---|
| `Rules/` | One test class per rule in `src/XamlQuality/Rules/`, each writing its markup into a fresh temporary directory; `FocusFakes.cs` and `PeerFakes.cs` are the miniature frameworks `BNXQ1005`'s and `BNXQ1006`'s tests read through reflection, and `HandedTemplateFakes.cs` holds the controls that hand their template to other code, for `BNXQ1003`. `UnloadableBuildOutputTests` covers the three rules that read compiled code, over `UnloadableAssembly.cs`: build output emitted at test time whose dependency does not load |
