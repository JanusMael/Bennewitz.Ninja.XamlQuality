...
|---|---|
| `Rules/` | One test class per rule in `src/XamlQuality/Rules/`, each writing its markup into a fresh temporary directory; `FocusFakes.cs` and `PeerFakes.cs` are the miniature frameworks `BNXQ1005`'s and `BNXQ1009`'s, and `BNXQ1006`'s, tests read through reflection. `ContainerNameFakes.cs` holds the automation peers `BNXQ1009` recognises a row by, in Avalonia's own namespace, and the item types its templates declare. `HandedTemplateFakes.cs` holds the controls that hand their template to other code, for `BNXQ1003`. `UnloadableBuildOutputTests` covers the four rules that read compiled code, over `UnloadableAssembly.cs`: build output emitted at test time whose dependency does not load |
...
| No test constructs a type in `Rules/FocusFakes.cs` | Constructing one runs its static constructor, which is `BNXQ1005`'s job; a test that ran it first would hide the cold read the rule has to get right | the file's header comment; test `KeyBindingFocusRuleTests.AListWithStockRows_IsLive_BecauseItsRowsTakeFocus` |
